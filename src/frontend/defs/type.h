...
type_spec_def *type_spec_def_new_primitive(type_id type_id) {
    return type_spec_def_new(type_id, false);
}

// create a new struct type specification
type_spec_def *type_spec_def_new_struct(type_id type_id) {
    return type_spec_def_new(type_id, true);
}

// create a new primitive array type specification
type_spec_def *type_spec_def_new_primitive_array(type_id type_id, size_t array_size) {
    type_spec_def *res = type_spec_def_new_primitive(type_id);
    res->is_array = true;
...
type_spec_def *type_spec_def_new_struct_array(type_id type_id, size_t array_size) {
    type_spec_def *res = type_spec_def_new_struct(type_id);
    res->is_array = true;
    res->array_size = array_size;
    return res;
}
